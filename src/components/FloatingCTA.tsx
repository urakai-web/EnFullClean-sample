  ...
  return (
    <div className="fixed bottom-5 right-4 md:bottom-8 md:right-8 z-50 flex flex-col items-end gap-2 md:gap-3">
      ...
      <button
        ...
        className="flex items-center gap-3 hover:opacity-95 active:scale-95 transition-all"
        aria-label={open ? 'メニューを閉じる' : 'お問い合わせ'}
      >
        {!open && (
          <div className="bg-navy text-white text-xs md:text-sm font-bold px-4 py-2 rounded-full shadow-lg relative">
            お問い合わせはこちら
            <div className="absolute top-1/2 -translate-y-1/2 -right-1.5 w-3 h-3 bg-navy rotate-45"></div>
          </div>
        )}
        <div className="relative">
          {open ? (
            <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-navy/80 shadow-2xl flex items-center justify-center">
            ...
            <img
              src="/images/Man.jpg"
              alt="お問い合わせ"
              className="w-16 h-16 md:w-20 md:h-20 rounded-full object-cover object-top shadow-2xl border-3 border-white"
            />
          )}
        </div>
      </button>
